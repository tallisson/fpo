...
void FpoReactive::SetErro(double erro) {
	m_erro = erro;
}

double FpoReactive::GetErro(void) const {
	return m_erro;
}

void FpoReactive::SetW(double w) {
	m_w = w;
}

double FpoReactive::GetW(void) const {
	return m_w;
}

void FpoReactive::SetMaxIter(double maxIter) {
	m_maxIter = maxIter;
}

double FpoReactive::GetMaxIter(void) const {
	return m_maxIter;
}

void FpoReactive::SetRo(double ro) {
	m_ro = ro;
}

double FpoReactive::GetRo(void) const {
	return m_ro;
}

void FpoReactive::SetVerbose(bool verbose) {
	m_verbose = verbose;
}

bool FpoReactive::GetVerbose(void) const {
	return m_verbose;
}

void FpoReactive::SetU(vec u) {
	m_u = u;
}

vec FpoReactive::GetU(void) const {
	return m_u;
}

void FpoReactive::Execute(std::string cdf) {
	m_lf->Prepare(cdf);
	...
	int numB = (int) graph->GetBuses().size();
	if (m_verbose) {
		printf("-----------------------------------------\n");
		printf("Iteração: \n");
	...
	while (flag == 0) {
		...
		if (conv == 1) {
			m_lf->GetJac()->Zeros(graph);
			mat jac = m_lf->GetJac()->CalcJac(graph);
			slack->CalcPG();
			if (m_verbose) {
			...
			if (conv == 1) {
				...
				double c = m_co;
				int numB = (int) graph->GetNumBus();
				Data_t bu = m_calc->Busca(m_lf, graph, m_w, c, m_erro, dLdu);
				cout << bu.print() << endl;
...
void FpoReactive::Report(LoadFlow* lf, int numIt) {
